# Agent Kit Workflow Audit

Date: 2026-07-28

This Agent-Native Research Artifact records the evidence and conclusions from
the independently reviewed audit of the repository's two-agent workflow.

## Layer Index

- [Problem](logic/problem.md)
- [Claims](logic/claims.md)
- [Heuristics](logic/solution/heuristics.md)
- [Exploration trace](trace/exploration_tree.yaml)
- [Evidence index](evidence/README.md)
- [Staged observations](staging/observations.yaml)
- [Session index](trace/sessions/session_index.yaml)

## Primary Artifact

The reader-facing result is
[`docs/audits/agent-workflow-audit.md`](../docs/audits/agent-workflow-audit.md).

## Additional Artifacts

- [`docs/tasks/002-fix-workflow-audit-findings.md`](../docs/tasks/002-fix-workflow-audit-findings.md):
  independently reviewed remediation and regression evidence.
- [`docs/research/cross-repository-agent-workflow-adoption.md`](../docs/research/cross-repository-agent-workflow-adoption.md):
  independently reviewed assessment of four target workflows and reciprocal
  integration candidates.
